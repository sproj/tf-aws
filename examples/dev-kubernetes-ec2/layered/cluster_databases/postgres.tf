terraform {
  ...
  backend "s3" {
    ...
    key            = "examples/kubernetes-ec2/cluster_databases/terraform.tfstate"
    region         = "eu-west-1"
...
resource "kubernetes_manifest" "postgres_external_secrets" {
  ...
  manifest   = yamldecode(file("./manifests/postgres-external-secrets.yaml"))
}

resource "kubernetes_manifest" "postgres_stateful_set" {
  depends_on = [kubernetes_namespace.databases_namespace]
  manifest   = yamldecode(file("./manifests/postgres-statefulset.yaml"))
}

resource "kubernetes_manifest" "postgres_headless_service" {
  depends_on = [kubernetes_namespace.databases_namespace]
  manifest   = yamldecode(file("./manifests/headless-service.yaml"))
}

resource "kubernetes_manifest" "postgres_service" {
  depends_on = [kubernetes_namespace.databases_namespace]
  manifest   = yamldecode(file("./manifests/postgres-service.yaml"))
}
