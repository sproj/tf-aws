...
cd cluster_databases/       && terraform apply
```
...
`cluster_databases` creates the `databases` namespace and deploys a shared Postgres instance. Superuser credentials are generated by `bootstrap/postgres-credentials/` and stored in SSM. Applications create their own database via an init container on first deployment, then connect to `postgres-service.databases.svc.cluster.local:5432`.
